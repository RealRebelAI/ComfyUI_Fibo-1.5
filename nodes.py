...
class FiboWanVAE:
    def __init__(self, weights_path, dtype="float32"):
...
class FiboVAELoader:
    ...
    def load(self, vae_name, dtype):
        path = folder_paths.get_full_path_or_raise("vae", vae_name)
        return (FiboWanVAE(path, dtype),)
...
print("[Fibo nodes v9-int8] registered:", ", ".join(NODE_CLASS_MAPPINGS.keys()))
